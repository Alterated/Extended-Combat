...
public class ModRecipeProvider extends RecipeProvider implements IConditionBuilder {
    ...
    protected void buildRecipes(Consumer<FinishedRecipe> pWriter) {
        ...
        ShapedRecipeBuilder.shaped(COMBAT, ModItems.DIAMOND_NAGINATA.get(), 1)
                .pattern(" AB").pattern(" CD").pattern("E  ")
                .define('A', Items.GOLD_NUGGET).define('B', Items.DIAMOND).define('C', Items.GOLD_INGOT).define('D', Items.GLOWSTONE_DUST).define('E', Items.STICK)
                .unlockedBy(getHasName(Items.GLOWSTONE_DUST), has(Items.GLOWSTONE_DUST))
                .save(pWriter);
        ...
        ShapedRecipeBuilder.shaped(COMBAT, ModBlocks.ROPE_BUNDLE.get(), 1)
                .pattern("AAA").pattern("AAA").pattern("AAA")
                .define('A', ModItems.ROPE.get())
                .unlockedBy(getHasName(Items.STRING), has(Items.STRING))
                .save(pWriter);
        ShapedRecipeBuilder.shaped(COMBAT, ModItems.ROPE.get(), 1)
                .pattern("AA ").pattern("ABA").pattern(" AA")
                .define('A', Items.STRING).define('B', Items.LEATHER)
                .unlockedBy(getHasName(Items.STRING), has(Items.STRING))
                .save(pWriter);
    }
}
